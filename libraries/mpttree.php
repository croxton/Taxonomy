...
/*	
		NEED TO FIX THE LEVEL ON THIS :(
		$sql = 'SELECT `n`.*, round((`n`.`'.$this->right_col.'` - `n`.`'.$this->left_col.'` - 1) / 2, 0) AS childs, 
			count(*) - 1 + (`n`.`'.$this->left_col.'` > 1) + 1 AS level, 
			((min(`p`.`'.$this->right_col.'`) - `n`.`'.$this->right_col.'` - (`n`.`'.$this->left_col.'` > 1)) / 2) > 0 AS lower, 
			(((`n`.`'.$this->left_col.'` - max(`p`.`'.$this->left_col.'`) > 1))) AS upper 
			FROM `'.$this->tree_table.'` `n`, `'.$this->tree_table.'` `p` 
			WHERE `n`.`'.$this->left_col.'` 
			BETWEEN `p`.`'.$this->left_col.'` 
			AND `p`.`rgt` 
			AND ( `p`.`node_id` != `n`.`node_id` OR `n`.`'.$this->left_col.'` = 1 )
			GROUP BY `n`.`node_id` 
			ORDER BY `n`.`'.$this->left_col.'`';
			
		*/	
		$sql = '
			SELECT `n`.*, exp_channel_titles.*, round((`n`.`'.$this->right_col.'` - `n`.`'.$this->left_col.'` - 1) / 2, 0) AS childs, 
			count(*) - 1 + (`n`.`'.$this->left_col.'` > 1) + 1 AS level,
			((min(`p`.`'.$this->right_col.'`) - `n`.`'.$this->right_col.'` - (`n`.`'.$this->left_col.'` > 1)) / 2) > 0 AS lower, 
			(((`n`.`'.$this->left_col.'` - max(`p`.`'.$this->left_col.'`) > 1))) AS upper 
			FROM `'.$this->tree_table.'` `n`
		...
		// this has issues and is returning extra items.
		$i = 0;
		$tmp = array();
	...
	// improved version... $root is lft value
	function get_flat_tree_v2($root = 1){
		...
		round((`n`.`'.$this->right_col.'` - `n`.`'.$this->left_col.'` - 1) / 2, 0) AS childs
	 ...
	 * but if no rows returned, empty array
	 */
    function mptree_cp_loop($array, $tagdata, $display_root = 'no', $depth = 10, $path = NULL, $entry_id = 0)
	{
    	...
    	foreach($array as $data)
    	{
    		...
    		if(($data['level'] == 0) && $display_root =="no")
    		{
    		 $str = $this->EE->mpttree->mptree_cp_loop($data['children'], $tagdata, $display_root, $depth, $path, $entry_id);
    		 $closing_ul = '';
    		}
    		...
    		else
    		{
				...
				$str .= '   <li class="node_'.$unique_class.' level_'.$data['level'].' '.$children_class.$active_parent.$active.'">';
				$str .= "";
	        	$str .= $this->EE->functions->var_swap($tagdata, $variables);
	        	
	  			
	        	if(isset($data['children']) && $data['level'] < $depth)
	        	{
		            $str .= $this->EE->mpttree->mptree_cp_loop($data['children'], $tagdata, $display_root, $depth, $path, $entry_id);
		        }
